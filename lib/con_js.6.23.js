...
$(function() {
   ...
   $("body").addClass((function(){
	...
	$("[change_nav]").each(function(){
       var nav_dom=$(this);
       nav_dom.children().click(function(){
       	  
       	   var ind=$(this).index();
   
       	   $(this).addClass("active").siblings().removeClass("active");
       	   nav_dom.next().children().eq(ind).show().siblings().hide();
       })
  	   nav_dom.children().eq(0).click();
	})
...
//ajax封装
var host="http://yxb.lebangcloud.cn/";
function go_ajax(param,callBack) {
    ...
    $.ajax({
        ...
        data:$.extend(param.data,param.noToken?{}:{token:localStorage.token||""}),//设置是否需要传token
    	   ...
    	   //分页加ID ，兼容IE8
			...
			var fenyeStyle=$("style#_pagination"+timeflag)[0].innerHTML;
			    fenyeStyle+=".SwiperSerial_"+_swiperSerial+"\t"+swiperDefaultParam.pagination+"\t*{margin:0 5px}\n";
	 	    	fenyeStyle+=".SwiperSerial_"+_swiperSerial+"\t"+swiperDefaultParam.pagination+"{position:absolute;width:100%;text-align:center;z-index:100;bottom:5px}\n";
	    	 ...
	    	 var swiper_params=$.extend({},swiperDefaultParam);
              if(SwiperDom.find(swiperDefaultParam.pagination).length)
              {
              	swiper_params.pagination=".fenye_"+_swiperSerial;
              }
              else{
              	delete swiper_params.pagination
              }
		     swiper_objs[_swiperSerial]=new Swiper(".SwiperSerial_"+_swiperSerial, swiper_params);
...
function GetUrlparam(name){
 ...
 function GetUrlAllparam(){
 	var params={};
 	var params_ary=location.search.substr(1);
 	params_ary=params_ary.split("&");
 	params_ary.forEach(function(item,index,ary){
 		params[item.split("=")[0]]=decodeURI(item.split("=")[1]);

 	})
 	window.URLparam=params;
 	return params;
 }
GetUrlAllparam()


//用iframe框架打开新页面，2个页面之间传参demo
function openFrame(url,hasBackBtn,data,backFunc){
		$("#_childrenFrame").length&&$("#_childrenFrame").remove()
		var html="<iframe  id='_childrenFrame' class='fixed_screenRight' src='"+url+"?parentFrame=true";
		html+=((hasBackBtn==true)?"&hasBackBtn=true":("&hasBackBtn="+hasBackBtn))
		if(data)
		{
		   for(var key in data)
		   {
		...
		html+=backFunc?("&_frameCallBack=true'></iframe>"):("'></iframe>");
	    $(html).appendTo($("body"))	
	    if(backFunc)
	    {

	    	$("body").append("<input type='hidden' id='_frameCallBack'>");
		    $("#_frameCallBack").unbind().click(function(){

		    	 var params=JSON.parse(sessionStorage[$("#_childrenFrame").attr("childFrameData-key")]);
		    	 backFunc(params)
		    	 sessionStorage.removeItem($("#_childrenFrame").attr("childFrameData-key"))
		    })
	    }

}
/******往父级iframe传参数,close_bool布尔值判断是否需要关闭当前iframe*******/
function sendParentData(data,close_bool){

	 //缓存中保存的子框架参数值的变量名字
	 var random="_childParam"+new Date().valueOf();
	 $(parent.document).find("#_childrenFrame").attr("childFrameData-key",random);
	 sessionStorage[random]=JSON.stringify(data);
	 $(parent.document).find("#_frameCallBack").click();
	 if(close_bool)
	 {
	 	$(parent.document).find("#_childrenFrame").removeClass("fixed_full").addClass("fixed_screenRight");
		setTimeout(function(){
			 $(parent.document).find("#_childrenFrame").remove()
		},500)
	 }
...
(function(){
	var parentFrame=GetUrlparam('parentFrame');
	var hasBackBtn=GetUrlparam('hasBackBtn');
	if(parentFrame&&(hasBackBtn!='false'))
	{
		if(hasBackBtn=='true')
		{
		  $("body>*.main").prepend("<div  id='_backFrame'  class='flex_dom flex_item_mid'><a class='fa fa-angle-left font30'></a><p class='flex_1 font28r'>返回</p></div>");
		   var backBtn="#_backFrame"
		}else{
		   var backBtn=hasBackBtn	
		}
		$(parent.document).find("#_childrenFrame").removeClass("fixed_screenRight").addClass("fixed_full")
		//页面加载完后绑定点击返回事件
        $("body").on("click",backBtn,function(){
        		$(parent.document).find("#_childrenFrame").removeClass("fixed_full");
        		setTimeout(function(){
        			 $(parent.document).find("#_childrenFrame").remove()
        		},500)
...
// 回到顶部
$(function () {
	var height = $("section > .overflow_auto").height()
	$("section > .overflow_auto").scroll(function (e) {
	...
	$(".go-top").click(function () {
		$("section > .overflow_auto").animate({ scrollTop: '0px' }, 300)
	})
})