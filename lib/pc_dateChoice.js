...
//JQ二次封装
$.fn.extend({
			dateChoice:function(params){
				var doms=$(this);
				var json=$.extend({
					type:"date",//默认三种选择模式，date,datetime,startend,	
					space_time:0,

				},params,{
					limitedDays:{
						startDate:params.limitedDays.todayStart?"today":params.startDate,
					}

				});
				json.limitedDays.limitedTimes=(params.limitedDays.limitedTimes>0?params.limitedDays.limitedTimes:Number.MAX_VALUE);

				doms.each(function(){
					var dom=$(this);
				    var params_obj=$.extend({},json);
					if(dom.attr("start-date")==""||dom.attr("end-date")=="")
					{
						$.extend(params_obj,{
							    format: 'yyyy-mm-dd',
                                pickTime: false
						...
						function dateChoiced(ev){
			           
					        var params_obj=$(this).data("initParams");
							var idx=$("[date-choice]").index($(this));
							var endtarget=$("[date-choice]").eq(idx+1);
							var endVal=endtarget.val();
							if(endVal)
							{
								
								
								endVal=endVal.split(/\W/);
								endVal.forEach(function(item,index,ary){
							   	...
							   	endVal=new Date(eval("new Date("+endVal.join(",")+").valueOf()")-params_obj.space_time*60*1000);
								$.extend(params_obj,{
									endDate:endVal.getFullYear()+"-"+(+endVal.getMonth()+1)+"-"+ endVal.getDate()
								})

							}
							var curr_date= new Date($(this).data("datepicker").date);
						    var new_obj= $.extend({},params_obj,{
						    	endDate:curr_date.getFullYear()+"-"+(+curr_date.getMonth()+1)+"-"+curr_date.getDate()
						    })
						    var newDom=$(this).clone();
						    newDom.insertAfter($(this));
						    $(this).remove();
					
						    newDom.fdatepicker(new_obj).on("changeDate",dateChoiced);
						    newDom.data("initParams",new_obj);
						    new_obj=$.extend({},endtarget.data("initParams"));
						    curr_date=new Date(curr_date.valueOf()+params_obj.space_time*60*1000)
						    $.extend(new_obj,{
						    	startDate:curr_date.getFullYear()+"-"+(+curr_date.getMonth()+1)+"-"+curr_date.getDate()
						    })
						    newDom=endtarget.clone();
						    newDom.insertAfter(endtarget);
						    endtarget.remove();
					
							if(new_obj.limitedDays.limitedTimes!=Number.MAX_VALUE&&new_obj.limitedDays.startDate=="today")
					        {
					            var item_obj=new Date(new Date().getFullYear(),new Date().getMonth(),new Date().getDate());
					            item_obj=new Date(item_obj.valueOf()+new_obj.space_time*60*1000)
					        	var endDay=new Date(item_obj.valueOf()+new_obj.limitedDays.limitedTimes*60*1000);
					        	$.extend(new_obj,{endDate:endDay.getFullYear()+"-"+(endDay.getMonth()+1)+"-"+endDay.getDate()});
					        	
					        }else if(new_obj.limitedDays.limitedTimes!=Number.MAX_VALUE){
					         
					     
					        	var item_obj=new_obj.startDate.split(/\W/);
					        	item_obj[1]--;
					        	item_obj=eval("new Date("+item_obj.join(",")+")");
					        	var endDay=new Date(item_obj.valueOf()+new_obj.limitedDays.limitedTimes*60*1000);
					        	$.extend(new_obj,{endDate:endDay.getFullYear()+"-"+(endDay.getMonth()+1)+"-"+endDay.getDate()});
	
					        }
					      
						    newDom.fdatepicker(new_obj).on("changeDate",dateChoiced2);
						    newDom.data("initParams",new_obj)
						    newDom.click()
							
							
						}
					function dateChoiced2(ev){
				            var params_obj=$(this).data("initParams");
							var idx=$("[date-choice]").index($(this));
							var starttarget=$("[date-choice]").eq(idx-1);
								var startVal=starttarget.val();
								
								if(endVal)
								{
									
									startVal=startVal.split(/\W/);
									startVal.forEach(function(item,index,ary){
								   	...
								   	startVal=new Date(eval("new Date("+startVal.join(",")+").valueOf()")+params_obj.space_time*60*1000) ;
									$.extend(params_obj,{
										startDate:startVal.getFullYear()+"-"+(+startVal.getMonth()+1)
	
									})
	
								}
								if(params_obj.limitedDays.limitedTimes!=Number.MAX_VALUE&&params_obj.limitedDays.startDate=="today")
						        {
						            var item_obj=new Date(new Date().getFullYear(),new Date().getMonth(),new Date().getDate());
						            item_obj=new Date(item_obj.valueOf()+params_obj.space_time*60*1000)
						        	var endDay=new Date(item_obj.valueOf()+params_obj.limitedDays.limitedTimes*60*1000);
						        	$.extend(params_obj,{endDate:endDay.getFullYear()+"-"+(endDay.getMonth()+1)+"-"+endDay.getDate()});
						        	
						        }else if(params_obj.limitedDays.limitedTimes!=Number.MAX_VALUE){
						        	
						     
						        	var item_obj=params_obj.startDate.split(/\W/);
						        	item_obj[1]--;
						        	item_obj=eval("new Date("+item_obj.join(",")+")");
						        	var endDay=new Date(item_obj.valueOf()+params_obj.limitedDays.limitedTimes*60*1000);
						        	$.extend(params_obj,{endDate:endDay.getFullYear()+"-"+(endDay.getMonth()+1)+"-"+endDay.getDate()});
							    ...
							    newDom.fdatepicker(new_obj).on("changeDate",dateChoiced2);
							    newDom.data("initParams",new_obj)
							    new_obj=$.extend({},starttarget.data("initParams"));
							    curr_date=new Date(curr_date.valueOf()-params_obj.space_time*60*1000)
							    $.extend(new_obj,{
							    	endDate:curr_date.getFullYear()+"-"+(+curr_date.getMonth()+1)+"-"+curr_date.getDate()
							    })
							    newDom=starttarget.clone();
							    newDom.insertAfter(starttarget);
							    starttarget.remove();
							    newDom.fdatepicker(new_obj).on("changeDate",dateChoiced);
							    newDom.data("initParams",new_obj)
					    }
						
						var ind=$("[date-choice]").index($(this));
						if(dom.attr("start-date")=="")
						{
							
							var today_obj=new Date();
							if(params_obj.startDate=="today")
							{
								
								$.extend(params_obj,{startDate:today_obj.getFullYear()+"-"+(+today_obj.getMonth()+1)+"-"+today_obj.getDate()});
							...
							if(params_obj.limitedDays.limitedTimes!=Number.MAX_VALUE&&params_obj.limitedDays.startDate=="today")
					        {
					        	var endDay=new Date(today_obj.valueOf()+params_obj.limitedDays.limitedTimes*60*1000);
					        	$.extend(params_obj,{endDate:endDay.getFullYear()+"-"+(endDay.getMonth()+1)+"-"+endDay.getDate()});
							...
							var endtarget=$("[date-choice]").eq(ind+1);
							var endVal=endtarget.val();
							if(endVal)
							{
							...
							dom.fdatepicker(params_obj).on('changeDate', dateChoiced);
					
							dom.data("initParams",params_obj)
						
							$("[date-choice]").eq(ind+1).focus();
							
						}else if(dom.attr("end-date")==""){
					
						
						    if(params_obj.startDate=="today")
							{
								var today_obj=new Date(new Date().valueOf()+params_obj.space_time*60*1000);
								$.extend(params_obj,{startDate:today_obj.getFullYear()+"-"+(+today_obj.getMonth()+1)+"-"+today_obj.getDate()})
								
							}else if(params_obj.startDate)
							{
								var today_obj=params_obj.startDate.split(/\W/);
								    today_obj[1]--;
								    today_obj=eval("new Date("+today_obj.join(",")+").valueOf()")
								    today_obj=new Date(today_obj+params_obj.space_time*60*1000);
								    $.extend(params_obj,{startDate:today_obj.getFullYear()+"-"+(+today_obj.getMonth()+1)+"-"+today_obj.getDate()})
							...
							var starttarget=$("[date-choice]").eq(ind-1);
							var startVal=starttarget.val();
							if(endVal)
							{
							 ...
							 dom.fdatepicker(params_obj).on('changeDate', dateChoiced2)
                             dom.data("initParams",params_obj)
						}
						...
						if(json.limitedDays.limitedTimes!=Number.MAX_VALUE&&json.limitedDays.startDate=="today")
				        {
	                          
	                            var endDay=new Date();
	                            endDay=new Date(new Date(endDay.getFullYear(),endDay.getMonth(),endDay.getDate()).valueOf()+json.limitedDays.limitedTimes*60*1000);
	                            $.extend(json,{endDate:endDay.getFullYear()+"-"+(+endDay.getMonth()+1)+"-"+endDay.getDate()});
		                         console.log(endDay.getFullYear()+"-"+(+endDay.getMonth()+1)+"-"+endDay.getDate());   
		                            
				        }else if(json.limitedDays.limitedTimes!=Number.MAX_VALUE&&json.limitedDays.startDate){
				        	  		
				        	  		var item_obj=json.startDate.split(/\W/);
						        	item_obj[1]--;
						        	item_obj=eval("new Date("+item_obj.join(",")+")");
						        	var endDay=new Date(item_obj.valueOf()+json.limitedDays.limitedTimes*60*1000);
						        	$.extend(json,{endDate:endDay.getFullYear()+"-"+(endDay.getMonth()+1)+"-"+endDay.getDate()});
